...
public class Tech_Manager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (string line_tower in lines_tower)
        {
            ...
            tech_tower.Add(new List<string> { words[2], words[3], "1" }); //이름, 설명, 레벨
        }

        foreach (string line_weapon in lines_weapon)
        {
            string[] words = line_weapon.Split('\t');
            tech_weapon.Add(new List<string> { words[2], words[3], "1" }); //이름, 설명, 레벨
        }
    ...
    public void Tech_Display_Panel()
    {
        ...
        left = tech_tower[rand_tower][1];
        //mid = tech_tower[rand_tower1];
        right = tech_weapon[rand_weapon][1];

        //타워
        icon_tower.sprite = Resources.Load<Sprite>("tower");
        image0.sprite = Resources.Load<Sprite>(tech_tower[rand_tower][0]);
        print(tech_tower[rand_tower][0]);
        name0.text = tech_tower[rand_tower][0];
        text0.text = left;

        //공용
        icon_public.sprite = Resources.Load<Sprite>("tower");
        image1.sprite = Resources.Load<Sprite>(tech_tower[rand_tower][0]);
        text1.text = "공용";

        //무기
        icon_weapon.sprite = Resources.Load<Sprite>("weapon");
        image2.sprite = Resources.Load<Sprite>("weapon");
        name2.text = tech_weapon[rand_weapon][0];
        text2.text = right;
    ...
    void display(string i)
    {
        ...
        {
            print(left);
            if (tech_tower[rand_tower][2] == "1")
            {
                tech_tower[rand_tower][2] = "2";
            }
            else if (tech_tower[rand_tower][2] == "2")
            {
                tech_tower[rand_tower][2] = "3";
            }
            else if (tech_tower[rand_tower][2] == "3")
            {
        ...
        {
            print(right);
            if (tech_weapon[rand_weapon][2] == "1")
            {
                tech_weapon[rand_weapon][2] = "2";
            }
            else if (tech_weapon[rand_weapon][2] == "2")
            {
                tech_weapon[rand_weapon][2] = "3";
            }
            else if (tech_weapon[rand_weapon][2] == "3")
            {
